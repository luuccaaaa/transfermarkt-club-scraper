...
def extract_from_url(tfmkt_url: Optional[str], element: str = "id") -> Optional[str]:
    ...
    trimmed_url = trim(tfmkt_url)

    try:
        match = re.match(regex, trimmed_url)
        groups: dict = match.groupdict() if match else {}
    except TypeError:
        groups = {}

    parsed_value: Optional[str] = groups.get(element)
    if parsed_value:
        return parsed_value
    ...
    def _first_valid(values: list[str]) -> Optional[str]:
        for value in values:
    ...
    fallback_keys = {
        "season_id": ["saison_id", "season_id"],
        "transfer_id": ["transfer_id"],
        "id": ["id"],
    }.get(element, [element])

    query_match = _first_valid([
        query_params.get(key, [None])[0] for key in fallback_keys if key in query_params
    ])
    if query_match:
        return query_match
    ...
    if element == "season_id":
        ...
        for idx, segment in enumerate(segments):
            if segment in {"saison_id", "season_id"} and idx + 1 < len(segments):
